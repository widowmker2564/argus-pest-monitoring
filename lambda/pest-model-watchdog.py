...
burns credits (~$4/hr, matching the dashboard's start-model warning in
web/dashboard_v4/js/settings.js:913). Triggered by an EventBridge rule
every 10 minutes.
...
def lambda_handler(event, context):
    ...
    for cam in cams:
        ...
        # v6.2: per-camera window so a scheduled morning run closes itself
        # ~45 min after start instead of the global default. The dashboard
        # writes max_runtime_min through the settings API.
        try:
            limit = int(cam.get("max_runtime_min", MAX_RUNTIME_MIN))
        except (TypeError, ValueError):
            limit = MAX_RUNTIME_MIN
        print(f"[Watchdog] {cid} RUNNING for {elapsed_min:.1f} min (limit {limit})")
