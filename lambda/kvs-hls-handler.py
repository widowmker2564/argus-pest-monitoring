...
Route (an API Gateway HTTP API route integrated to THIS function):
  GET /video-playback?stream=<kvs_stream_name>

Account status (2026-08-21). This is the repo mirror of the deployed function;
the as-deployed artifact is deployer/audit/kvs-hls-handler_src/. The function
and its route were built and validated on the retired development account
(gateway `zwpcbivmsj`). The production account `506868652945` was deployed
WITHOUT `--live-view`, so it has no KVS streams and the Live tab has nothing to
play there yet. Create a stream on production before wiring this up; see
reports/manual/01_system_architecture.md section 1.6.
