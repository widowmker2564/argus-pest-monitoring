# Deploy manifest — authoritative BOM of the live nbk2 stack

_Built by auditing the LIVE account 366356442579 / us-east-1 (profile `nbk2`),
NOT from docs (docs/aws.md was found materially out of date — it said 3 Lambdas /
4 tables; reality is 8 Lambdas / 6 pest tables / 3 API GWs). This file is the
input to `deploy.py`: everything here must be recreatable, parameterized, in a
fresh account. Started 2026-07-08._

## Scope note — SHARED ACCOUNT
nbk2 hosts unrelated projects too (amplify chatbot, projassist, sagemaker,
unit-psychologist, lights-out, spf* chatbot). deploy.py must cherry-pick ONLY the
pest resources listed here. Non-pest buckets/roles are explicitly out of scope.

## Classification legend
- **[PROD]** load-bearing for the fixed-camera product (the deployer target)
- **[DEMO]** demo/testbed only (Go2 patrol, KVS live view) — include or drop per design
- **[DEAD]** legacy / superseded — do NOT recreate; candidate for cleanup
- **[?]** not yet classified (audit in progress)

## PRODUCT SCOPE RULING — Runzhe, 2026-07-08 (governs everything below)
The deployer ships a **generic, detection-target-agnostic product**, minimal set only:

1. **Only function-bearing config migrates.** Anything not serving the product
   function is out — no legacy code, no demo instances, no historical artifacts.
2. **NO dataset ships.** The product is open: the customer brings their OWN images
   for their OWN target (any pest/object, not necessarily armyworm). We ship the
   labelling + YOLO→Rekognition conversion tooling and the empty project/dataset
   skeleton; training data never migrates. (The system is already target-agnostic
   in code: detection target = per-camera `target_label` config; nothing hardcodes
   armyworm in the Lambdas/dashboard logic.)
3. **Genericize all armyworm-flavored NAMING into parameters:** bucket names
   (`frames-armyworm-*` → `<PRODUCT_PREFIX>-frames-<ACCT>` or similar), Rekognition
   project name, DDB tag values, the camera seed's target_label — all wizard inputs.
4. **Camera seed shrinks to 2 rows:** `manual_upload` (required by the dashboard
   Test-upload feature) + ONE template camera whose name/target_label/kvs stream the
   wizard fills. The nbk2 rows person_cam (test artifact) and moth_cam_01 (demo) do
   NOT ship.
5. **Open decisions §5 — RESOLVED by this ruling:**
   - #1 scheduled auto-capture: do NOT port the Gen-1 daily trio. The Gen-2 dynamic
     per-camera schedule mechanism (api creates `pest-sched-*` rules at runtime) IS
     the product's scheduling; nothing to pre-create beyond role/permissions.
   - #2 moth pipeline: OUT. Single generic pipeline; moth was a demo instance.
   - #3 image fetch path: fresh accounts serve images exclusively via presigned
     frames-bucket URLs (processed bucket stays empty on v4.2+); frames CORS gets
     the new origins at write-back. No extra work.
   - #4 EC2: OUT. Dashboard is S3+CloudFront; deploy.py provisions no EC2.
   - #6 model auto-start: NO — leave STOPPED after training; wizard shows cost and
     lets the customer start it (watchdog guards runaway cost either way).
   - #8 kvs-hls-url-generator: DEAD confirmed; DROP its InvokeFunction grant from the
     recreated api-role policy (stale reference to a superseded Gen-1 function).
6. **Additional cuts from the PROD set:** `lambda-layers-<ACCT>` bucket (only held the
   unused Gen-1 ffmpeg layer; fyp-pillow at 7.9 MB publishes directly, no S3 staging)
   and the ffmpeg layer itself. `processed-images` bucket: KEEP creating it (the
   processor/api cold-start REQUIRE the env var and the delete path references it)
   but note it stays empty on fresh installs — v4.2+ never writes processed images.
7. **KVS streams:** create only the template camera's stream, and only if the wizard
   enables live view; stream names are wizard inputs, not `armyworm-cam-stream`.

Still genuinely open: none blocking deploy.py. (#5 Cognito PreventUserExistenceErrors:
keep live default for faithfulness; #7 runtime scheduler role: deploy.py creates the
stably-named invocation role — done by design.)

---

## Deep audit — RESOLVED 2026-07-08 (this supersedes the first-pass inventory)
...
|---|---|---|---|
| **pest-detection-processor-role** | `AWSLambdaBasicExecutionRole` | inline `pest-detection-processor-policy` (S3 GetObject on frames-armyworm/\*, Get+Put on processed-images-armyworm/\*, DDB PutItem/GetItem/UpdateItem/Query on detections+index/\*, GetItem on cameras, `rekognition:DetectCustomLabels` \*, `ses:SendEmail/SendRawEmail/ListVerifiedEmailAddresses` \*); inline `read-system-config` (DDB GetItem on system-config); inline `bedrock-verify` (`bedrock:InvokeModel` + `bedrock:Converse` on the account's `inference-profile/us.anthropic.*` etc. + underlying `foundation-model/*` — REQUIRED since the LLM verify gate, 2026-07-21; without it the fail-closed gate rejects every detection); inline `s3-frames-write` (`s3:PutObject` on frames-armyworm/frames/\* for EXIF-normalised write-back) | `iam__trust-policy-lambda.json`, `iam__pest-detection-processor-role__inline__pest-detection-processor-policy.json`, `iam__pest-detection-processor-role__inline__read-system-config.json`, `iam__pest-detection-processor-role__inline__bedrock-verify.json` (captured 2026-08-11), `iam__pest-detection-processor-role__inline__s3-frames-write.json` (captured 2026-08-11) |
...
| **pest-dashboard-\<ACCT\>** | Versioning OFF; SSE-S3 AES256; **PAB all FALSE**; **public-read policy** `PublicReadDashboard` (Principal \*, s3:GetObject on `/*`); **website** Index=index.html Error=index.html (SPA fallback); no CORS. Sync 15 files from `web/dashboard_v4/` (index.html, styles.css, js/{analytics,api,auth,bbox,config,costs,gallery,live,main,modal,settings,state,utils}.js) | `s3__pest-dashboard__policy.json`, `s3__pest-dashboard__website.json`, `s3__pest-dashboard__public-access-block.json` |
...
`lambda/` are named `pest-*.py` and **must be renamed to `lambda_function.py`**
inside the zip.

| Function | Mem / Timeout | Role | Layers | Env vars | Trigger | Raw files |
|---|---|---|---|---|---|---|
| **pest-detection-processor** | 1024 / 600 (validated production sizing: tiling + up to 120 Bedrock verify crops) | pest-detection-processor-role | fyp-pillow:\<N\> | `SENDER_EMAIL=<CUSTOMER_EMAIL>`, tables, `S3_PROCESSED_BUCKET`, plus the validated detection tuning deploy.py ships: `TILE_MIN_CONFIDENCE=8`, `LLM_VERIFY_ALL_BOXES=true`, `LLM_VERIFY_MODEL_ID=us.anthropic.claude-sonnet-4-6`, `LLM_VERIFY_MAX_BOXES=120`, `LLM_VERIFY_MAX_TOKENS=300`, `LLM_VERIFY_WORKERS=3`, `LLM_VERIFY_PAD=0.6`, `POST_NMS_IOU=0.1`, `POST_NMS_CONTAIN=0.1`, `POST_MAX_BOX_AREA=0.05`, `POST_VERIFY_FLOOR=49` | S3 frames-armyworm ObjectCreated:\* prefix `frames/` | `lambda__pest-detection-processor.json` (512/60 snapshot is HISTORICAL — live is 1024/600 since 2026-08) |
...
| **pest-model-watchdog** | 128 / 30 | pest-model-watchdog-role | none | `TABLE_CAMERAS`, `MAX_RUNTIME_MIN=75` (global fallback; the v6.2 code honours per-camera `max_runtime_min`, e.g. 45 on the seeded camera) | EventBridge Scheduler `pest-model-watchdog-15min` | deploy.py ships `lambda/pest-model-watchdog.py` (v6.2, per-camera window) — the `pest-model-watchdog_src/` audit snapshot is the pre-v6.2 HISTORICAL copy; `lambda__pest-model-watchdog.json`, `scheduler__pest-model-watchdog-15min.json` |

> **Two code-mirror gaps closed during this audit:** kvs-hls-handler and
> pest-model-watchdog live code were NOT in `lambda/` (only the 3 core were).
> Both were downloaded to `deployer/audit/<name>_src/lambda_function.py`. Ideally
> copy them into `lambda/` so the source of truth is one place.
...
10. **Config write-back** — rewrite `web/dashboard_v4/js/config.js` (`HTTP_API`=new API base URL, `COGNITO_REGION`, `COGNITO_CLIENT_ID`); update frames-bucket CORS AllowedOrigins with the new CloudFront domain + dashboard website endpoint; then `aws s3 sync web/dashboard_v4/ s3://pest-dashboard-<ACCT>/`.
